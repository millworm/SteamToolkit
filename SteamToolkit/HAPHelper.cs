...
namespace BotDataScrapeTools
{
    public class HapHelper
    {
        ...
        {
            return node.Descendants("div").Where(d => d.Attributes.Contains(attribute)).Where(d => d.Attributes[attribute].Value.ToUpper().Contains(attributeValue.ToUpper())).ToList();
        }
        ...
        public static List<HtmlNode> FindAllNodesByAttribute(HtmlNode start, string name    , string value)
        {
            ...
            foreach (HtmlNode node in start.ChildNodes)
            {
                ...
                {
                    matchingNodes.AddRange(from attribute in node.Attributes where attribute.Name.ToUpper().Contains(name.ToUpper()) && attribute.Value.ToUpper().Contains(value.ToUpper()) select node);
                }
